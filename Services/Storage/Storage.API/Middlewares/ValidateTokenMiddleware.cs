namespace BIVN.FixedStorage.Services.Storage.API.Middlewares
{
    public class ValidateTokenMiddlware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            var ClientId = context.Request.Headers[Constants.HttpContextModel.ClientIdKey].FirstOrDefault()?.Split(" ").Last();
            var ClientSecret = context.Request.Headers[Constants.HttpContextModel.ClientSecretKey].FirstOrDefault()?.Split(" ").Last();
            ...
            var token = context.Request.Headers[Constants.HttpContextModel.AuthorizationKey].FirstOrDefault()?.Split(" ").Last();
            var deviceId = context.Request.Headers[Constants.HttpContextModel.DeviceId].LastOrDefault();
            ...
            restRequest.AddHeader(Constants.HttpContextModel.AuthorizationKey, $"Bearer {token}");
            restRequest.AddHeader(Constants.HttpContextModel.DeviceId, deviceId ?? "");
            ...
            try
            {
                ...
                //Token passed
                context.Items[Constants.HttpContextModel.UserKey] = responseModel.Data;
                ValidateTokenResultDto User = responseModel.Data;
                var claimIdentity = new ClaimsIdentity(authenticationType: Constants.HttpContextModel.UserKey);
                var claim = new[]
                {
        ...
        {
            string appClientIdValue = _configuration[Constants.AppSettings.ClientId] ?? string.Empty;
            string appClientSecretValue = _configuration[Constants.AppSettings.ClientSecret] ?? string.Empty;
