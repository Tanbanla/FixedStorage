...
var RoleController = (function () {
    let root = {
        parentEl: $(".Views_Roles_Index"),
    }

    //function PrepareDeleteAPI(roleId, userId) {
    //    return new Promise(async (resolve, reject) => {
    //        let url = `${host}/api/identity/role/prepare/${deparmentId}/${userId}`
    //        try {
    ...
    function DeleteAPI(roleId, userId) {
        return new Promise(async (resolve, reject) => {
            let url = `role/delete/${roleId}/${userId}`
            //let url = `${host}/api/identity/role/delete/${roleId}/${userId}`
            try {
                const res = await $.ajax({
                    url: url,
                    type: 'DELETE',
    ...
    function PerpareDeleteAPI(roleId) {
        return new Promise(async (resolve, reject) => {
            let url = `${host}/api/identity/role/delete/prepare/${roleId}`
            try {
                const res = await $.ajax({
                    url: url,
                    type: 'GET',
    ...
    function PrepareEditRoleInfo(roleId) {
        return new Promise(async (resolve, reject) => {
            let url = `role/prepare/${roleId}`
            try {
                const res = await $.ajax({
                    url: url,
                    type: 'GET',
    ...
    function RoleListAPI() {
        return new Promise(async (resolve, reject) => {
            try {
                let url = `${host}/api/identity/role`
    ...
    function Events() {
        root.openCreateRoleModal.click((e) => {
            RefreshCreateModalData()

            root.createModal.modal("show")
        })

        root.deleteRoleIcon.click((e) => {
            let roleId = $(e.target).closest(".btnDeleteRole").data("roleid");
            let userId = $("#App_UserId").val(); 

            Swal.fire({
                title: `<b>${window.languageData[window.currentLanguage]["Xác nhận xóa"]}</b>`,
                text: window.languageData[window.currentLanguage]["Bạn có chắc chắn muốn xóa nhóm quyền này?"],
                ...
                if (result.isConfirmed) {
                    loading(true);
                    DeleteAPI(roleId, userId).then(res => {
        ...
        root.btnOpenEditModal.click((e) => {
            let target = $(e.target)
            let roleId = target.closest(".btnEditRole").data("roleid")
            let parentItemContainer = $(target).closest(`.role_child [data-roleid='${roleId}']`)

            //Assign role to modal
            root.editModal.data("roleId", roleId)

            let info = rolesList.find(x => x.RoleId == roleId)
            ...
            //Fill edit data
            //Fill name
            let roleName = info?.Name
            root.editModal.find('#Name').val(roleName || "")
            root.editModal.find('#Name').attr("data-root", roleName)


            //Fill website access
            let webAccessToggle = root.editModal.find(".WebAccessToggle")
            let hasWebAccess = permissions?.some(item => item?.Name === webAccessToggle?.val())
            if (hasWebAccess) {
                webAccessToggle.trigger("click")
            }
            //Fill mobile access
            let mobileAccessToggle = root.editModal.find(".MobileAccessToggle")
            let hasMobileAccess = permissions?.some(item => item?.Name === mobileAccessToggle?.val())
            if (hasMobileAccess) {
                mobileAccessToggle.trigger("click")
            }

            root.editModal.find("[name='permission[]']:not(.WebAccessToggle, .MobileAccessToggle)")?.map((i, el) => {
                let elVal = $(el).data("key")                
                let elCategory = $(el).data("category");            
                let isChecked;
                if (elCategory == 'DEPARTMENT_DATA_INQUIRY' || elCategory == 'FACTORY_DATA_INQUIRY' || elCategory == 'CREATE_DOCUMENT_BY_DEPARTMENT') {
                    isChecked = permissions?.some(item => (item?.Category == elCategory && item?.Name.toUpperCase() == elVal))
                } else {
                    isChecked = permissions?.some(item => item?.Name == elVal || (item?.Category == elCategory && item?.Name.toUpperCase() == elVal))
                }
                $(el).prop("checked", isChecked)
            })

            root.editModal.modal("show")
        })
    ...
    function RefreshEditModalData() {
        root.editModal.find('#Name').val("")

        root.editModal.find(".WebAccessToggle, .MobileAccessToggle").prop("checked", false)
        root.editModal.find("[name='permission[]']:not(.WebAccessToggle, .MobileAccessToggle)").prop("checked", false)
        root.editModal.find("[name='permission[]']:not(.WebAccessToggle, .MobileAccessToggle, #ViewByDepartment [name='permission[]'])").attr("disabled", true)
        root.editModal.find("[name='permission[]']:not(.WebAccessToggle, .MobileAccessToggle, #ViewByDepartment [name='permission[]'])").closest(".form-check").removeClass("role_uncheck").addClass("role_uncheck")
    }

    function RefreshCreateModalData() {
        root.createModal.find('#Name').val("")
        root.createModal.find(".WebAccessToggle, .MobileAccessToggle").prop("checked", false).change()
        root.createModal.find("[name='permission[]']:not(.WebAccessToggle, .MobileAccessToggle)").prop("checked", false)
    }
                            ...
                            <div class="role_item h100 rounded role_child px-3 py-2 " data-roleid="@role?.RoleId">
                                ...
                                <div class="row create_website_access mt-2">
                                    <div class="col-12 d-flex justify-content-between">
                                        <div>
                                            <label class="txt-bolder txt-14">Website</label>
                                        </div>
                                        <div>
                                            @{
                                ...
                                <div class="row create_mobile_access">
                                    <div class="col-12 d-flex justify-content-between">
                                        <div>
                                            <label class="txt-bolder txt-14">App mobile</label>
                                        </div>
                                        <div>
                                            @{
                                ...
                                <h5 class="h5 txt-bolder txt-14">Phân quyền xem dữ liệu</h5>
                                @{
                                ...
                                @if (filterDepartmentNames?.Any() == true)
                                {
                                    <div class="check_list">
                                        ...
                                        <div>
                                            <text class="txt-14 py-1 txt-green">Xem dữ liệu theo phòng ban</text>
                                            <div class="txt-14 txt-wrap">@Html.Raw(string.Join(", ", filterDepartmentNames))</div>
                                        </div>
                                    </div>
                                ...
                                @if (filterDepartmentNames?.Any() == false)
                                {
                                    <div class="unchecked_task">
                                        ...
                                        <text class="txt-14 py-1">Xem dữ liệu theo phòng ban</text>
                                    </div>
                                }
                                ...
                                @if (filterFactoryNames?.Any() == true)
                                {
                                    <div class="check_list">
                                        ...
                                        <div>
                                            <text class="txt-14 py-1 txt-green">Xem dữ liệu theo nhà máy</text>
                                            <div class="txt-14 txt-wrap">@Html.Raw(string.Join(", ", filterFactoryNames))</div>
                                        </div>
                                    </div>
                                }
                                ...
                                @if (filterFactoryNames?.Any() == false)
                                {
                                    <div class="unchecked_task">
                                        ...
                                        <text class="txt-14 py-1">Xem dữ liệu theo nhà máy</text>
                                    </div>
                                }
                            </div>
                         `
    ...
    function Cache() {
        root.openCreateRoleModal = root.parentEl.find(".btnNewRole")
        root.createModal = $(root.parentEl.find("#Views_Shared_Common_Role__CreateRoleView"))
        root.createForm = root.createModal.find("#createRoleForm")

        root.deleteRoleIcon = root.parentEl.find(".btnDeleteRole")

        root.btnOpenEditModal = root.parentEl.find(".btnEditRole")
        root.editModal = $(root.parentEl.find("#Views_Shared_Common_Role__EditRoleView"))
    }

    function PreLoad() {
        if (actionMessage?.data) {
            toastr.success(window.languageData[window.currentLanguage][actionMessage.message]);
        } else if (actionMessage?.data == false) {
            toastr.error(window.languageData[window.currentLanguage][actionMessage.message]);
        }
    }

    function Init() {
        if (root.parentEl?.length <= 0) {
            console.error("Không tìm thấy Role container")  
        }
